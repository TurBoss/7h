        ...
        public static void DownloadAndInstall(Mod m) {
            ...
            if (install != null) {
                ...
                if (patches.Any()) {
                    ...
                    Sys.Downloads.Download(patches.Select(p => p.Link),
                        ...
                        "Downloading " + m.Name,
                        new PatchModProcedure() { File = pfile, Install = install, Mod = m },
            ...
            if (m.LatestVersion.PatchLinks.Any()) {
                ...
                Sys.Downloads.Download(m.LatestVersion.Links,
                    ...
                    "Downloading " + m.Name,
                    new DownloadThenPatchProcedure() { File = file, Mod = m, Controller = pc },
                    () => Sys.RevertStatus(m.ID)
                );
                int pCount = 0;
                foreach (string p in m.LatestVersion.PatchLinks) {
                    ...
                    Sys.Downloads.Download(p,
                        ...
                        new DownloadPatchProcedure() { File = pfile, Controller = pc },
                        () => {
                            pc.PatchFailed();
                            Sys.RevertStatus(m.ID);
                        }
                ...
                Sys.Downloads.Download(m.LatestVersion.Links,
                    System.IO.Path.Combine(temppath, file),
                    "Downloading " + m.Name,
                    new InstallModProcedure() { File = file, Mod = m, ExtractSubFolder = m.LatestVersion.ExtractSubFolder, ExtractInto = m.LatestVersion.ExtractInto },
                    () => Sys.RevertStatus(m.ID)
                );
            }
        }
            ...
            private void ProcessDownload(object state) {
                try {
                    ...
                    using (var fs = new System.IO.FileStream(source, System.IO.FileMode.Open, System.IO.FileAccess.Read)) {
                        ...
                        if (isig == _7thWrapperLib.IrosArc.SIG) { //plain IRO file, just move into place
                            fs.Close();
