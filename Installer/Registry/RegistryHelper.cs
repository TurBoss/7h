...
namespace Installer.Registry
{
    ...
    public static class RegistryHelper
    {
        ...
        {
            string testPath = RegistryHelper.GetValue(RegistryHelper.SteamKeyPath32Bit, "InstallLocation", "") as string;
            if (!string.IsNullOrWhiteSpace(testPath))
            {
                return testPath;
            }

            testPath = RegistryHelper.GetValue(RegistryHelper.SteamKeyPath64Bit, "InstallLocation", "") as string;
            if (!string.IsNullOrWhiteSpace(testPath))
            {
                return testPath;
            }
            ...
            testPath = RegistryHelper.GetValue(RegistryHelper.RereleaseKeyPath32Bit, "InstallLocation", "") as string;
            if (!string.IsNullOrWhiteSpace(testPath))
            {
                return testPath;
            }

            testPath = RegistryHelper.GetValue(RegistryHelper.RereleaseKeyPath64Bit, "InstallLocation", "") as string;
            if (!string.IsNullOrWhiteSpace(testPath))
            {
                return testPath;
            }

            testPath = RegistryHelper.GetValue(FF7RegKey.FF7AppKeyPath, "Path", "") as string;
            if (!string.IsNullOrWhiteSpace(testPath))
            {
                return testPath;
            }
